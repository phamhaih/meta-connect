import { createActor } from "../../declarations/MetaConnect_backend";
    ...
    actor(orRedirect: boolean = true): BackendActor | undefined {
      if (!this.backendActor && client) {
        ...
        if (identity.getPrincipal().toString().length > 10) {
          ...
          this.backendActor = createActor(config.canisterIds.MetaConnect_backend, {
            agent,
          });
        }
      }
...
export const anonActor = (): BackendActor => {
  if (!inBrowser()) throw "Please run in browser";
  if (useAuthState().loggedIn) {
    return useAuthState().actor(false)!;
  }
  const config = useRuntimeConfig().public;
  const agent = new HttpAgent({ host: config.host });
  (window as any).global = window;
  const anon = createActor(config.canisterIds.MetaConnect_backend, { agent });
  return anon;
};

let client: AuthClient | null = null;

export const loginTest = (): PRes<AuthClient> => {
  const config = useRuntimeConfig().public;
