import { MetaConnect_backend } from "../../declarations/MetaConnect_backend";
export type * from "../../declarations/MetaConnect_backend/MetaConnect_backend.did";
export type BackendActor = typeof MetaConnect_backend;
...
import type {
  ...
  Message,
  ChatStatus,
} from "../../declarations/MetaConnect_backend/MetaConnect_backend.did";
...
const effectify = <T>(
  fn: (actor: BackendActor) => Promise<T>,
  orRedirect: boolean = true,
): FrontendEffect<T> => {
...
const effectifyResult = <T>(
  fn: (actor: BackendActor) => Promise<{ ok: T } | { err: BackendError }>,
  orRedirect: boolean = true,
): FrontendEffect<T> => {
  return Effect.gen(function* (_) {
...
export const getAnsweredQuestions = (
  team: string,
): FrontendEffect<[Question, Answer][]> => {
...
export const loadUser = (
  orRedirect = true,
): FrontendEffect<UserPermissions> => {
...
export const answerQuestion = (
  ...
  a: boolean,
  boost: number,
): FrontendEffect<Answer> => {
  return effectifyResult((actor) =>
    actor.submitAnswer(team, q.valueOf(), a, BigInt(boost)),
  );
};
...
export const loadQuestionStats = (
  team: string,
): FrontendEffect<QuestionStats[]> => {
  const limit = BigInt(200);
...
export const friendStatusIcon = (
  ...
  switch (friendStatusToKey(status)) {
    case "connected":
      return { icon: "tabler:user-check", color: "text-green-500" };
    case "rejectionReceived":
      return { icon: "tabler:user-x", color: "text-red-500" };
    case "rejectionSend":
      return { icon: "tabler:user-cancel", color: "text-red-600" };
    case "requestReceived":
      return { icon: "tabler:user-question", color: "text-orange-500" };
    case "requestSend":
      return { icon: "tabler:user-exclamation", color: "text-gray-500" };
  }
  console.error("Unexpected friend status", status);
  return { icon: "?", color: "" };
};
...
export const answerFriendRequest = (
  ...
  return effectifyResult((actor) =>
    actor.answerFriendRequest(team, username, accept),
  );
};
...
export const joinTeam = (
  ...
  return effectifyResult((actor) =>
    actor.joinTeam(team, code, invitedBy ? [invitedBy] : []),
  );
};
...
export const createTeam = (
  ...
  return effectifyResult((actor) =>
    actor.createTeam(team, code, { name, about, logo, listed }),
  );
};
...
export const updateTeam = (
  ...
  return effectifyResult((actor) =>
    actor.updateTeam(team, code, { name, about, logo, listed }),
  );
};
...
export const getMessages = (
  team: string,
  user: string,
): FrontendEffect<{ messages: Message[]; status: ChatStatus }> => {
